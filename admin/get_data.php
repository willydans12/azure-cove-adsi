...
// Booking list
$bq = $mysqli->query("SELECT booking_id, guest_name, guest_email, DATE_FORMAT(check_in,'%d %b') AS check_in, DATE_FORMAT(check_out,'%d %b') AS check_out, room, CONCAT('Rp ',FORMAT(total,0,'de_DE')) AS total, status FROM bookings ORDER BY check_in DESC LIMIT 10");
